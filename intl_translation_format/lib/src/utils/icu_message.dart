...
class _IcuParser {
  ...
  Parser get pluralClause =>
      (pluralKeyword & openCurly & interiorText & closeCurly).trim().map((result) => [result[0], result[2]]);
  ...
  Parser get genderClause =>
      (genderKeyword & openCurly & interiorText & closeCurly).trim().map((result) => [result[0], result[2]]);
  ...
  Parser get message => (pluralOrGenderOrSelect | simpleText | empty).map((chunk) => Message.from(chunk, mainMessage));
...
String _turnInterpolationIntoICUForm(Message message, chunk, {bool shouldEscapeICU = false}) {
  if (chunk is String) {
  ...
  if (chunk is SubMessage) {
    return chunk.expanded((message, chunk) => _turnInterpolationIntoICUForm(message, chunk, shouldEscapeICU: true));
  }
  if (chunk is Message) {
    return chunk
        .expanded((message, chunk) => _turnInterpolationIntoICUForm(message, chunk, shouldEscapeICU: shouldEscapeICU));
  }
